    ...
    <Flex h="100vh" bg="green.50">
      ...
      <Flex flex='1' direction='column'>
        ...
        <Box flex='1' overflowY='auto' display='flex' alignItems='center' flexDirection='column' position='relative' bg='green.50'>
          <VStack spacing={4} align='stretch' w='100%' px={{base:6, sm:12, md:16}} py={4} pb={16}>
              ...
              return (
                <Flex key={idx} justify={isSender ? 'flex-end' : 'flex-start'} position='relative' pb={4} align='center'>
                  ...
                  <Box px={4} py={2} rounded='xl' maxW={{ base: '80%', md: '60%', lg: '45%' }} bg={isSender ? 'green.500' : 'white'} color={isSender ? 'white' : 'gray.800'}>
                    {msg.image && <img src={msg.image} alt='sent' style={{ width: '100%', maxHeight: '300px', borderRadius: 8, marginBottom: msg.text ? 8 : 0, objectFit: 'cover' }} onClick={() => setOpenImage(msg.image)} />}
        ...
        {selectedImage && <Box mb={2} display='flex' flexDirection='column' alignItems='center' p={2} bg='green.100'><img src={selectedImage} alt='preview' style={{ maxWidth:'350px', borderRadius: 8 }} /><Button size='sm' mt={2} colorScheme='red' variant='outline' onClick={() => setSelectedImage(null)}>Remove</Button></Box>}
        <Flex p={4} bg='white' width="100%" borderTop='1px' borderColor='green.200' gap={2}>
